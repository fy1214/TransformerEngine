...
// Same tiling as multi_padding_kernel, plus companion fp32 per-row amax
// (written by leftmost tiles only). Original multi_padding_kernel is unchanged.
struct MultiPaddingWithAmaxArgs {
  void* input_list[kMaxTensorsPerKernel];
  void* output_list[kMaxTensorsPerKernel];
  float* amax_input_list[kMaxTensorsPerKernel];
  float* amax_output_list[kMaxTensorsPerKernel];
  int num_rows_list[kMaxTensorsPerKernel];
  int padded_num_rows_list[kMaxTensorsPerKernel];
  int row_length_list[kMaxTensorsPerKernel];
  int block_range[kMaxTensorsPerKernel + 1];
...
__global__ void __launch_bounds__(threads_per_block)
    multi_padding_with_amax_kernel(MultiPaddingWithAmaxArgs args) {
  using Vec = Vec<Type, nvec>;
  ...
  const Type* input = reinterpret_cast<const Type*>(args.input_list[tensor_id]);
  Type* output = reinterpret_cast<Type*>(args.output_list[tensor_id]);
  const float* amax_input = args.amax_input_list[tensor_id];
  float* amax_output = args.amax_output_list[tensor_id];
  const int num_rows = args.num_rows_list[tensor_id];
  const int padded_num_rows = args.padded_num_rows_list[tensor_id];
  const int row_length = args.row_length_list[tensor_id];
...
#pragma unroll
  ...
  // Companion fp32 row-amax outside the activation hot loop (leftmost tiles only).
  if (tile_id_n == 0 && tidx == 0) {
    for (int iter = 0; iter < n_iterations; ++iter) {
      const int i1 = tidy + iter * bdimy;
#pragma unroll
      for (int i2 = 0; i2 < nvec; ++i2) {
        const int row = tile_row + i1 * nvec + i2;
        if (row < num_rows) {
          amax_output[row] = amax_input[row];
        } else if (row < padded_num_rows) {
          amax_output[row] = 0.f;
        }
      }
...
void multi_padding_with_amax(const std::vector<Tensor*> input_list,
                             std::vector<Tensor*> output_list, const float* amax_input,
                             float* amax_output, const std::vector<int> padded_num_rows_list,
                             cudaStream_t stream) {
  NVTE_CHECK(output_list.size() == input_list.size(),
             "Number of input and output tensors must match");
  NVTE_CHECK(amax_input != nullptr && amax_output != nullptr,
             "amax_input/amax_output must be non-null");
  if (input_list.empty()) {
    return;
  }
  ...
  for (size_t tensor_id = 0; tensor_id < input_list.size(); ++tensor_id) {
    const auto& input = *input_list[tensor_id];
    const auto& output = *output_list[tensor_id];
    CheckInputTensor(input, "multi_padding_with_amax_input_" + std::to_string(tensor_id));
    CheckInputTensor(output, "multi_padding_with_amax_output_" + std::to_string(tensor_id));
  ...
  // Flat amax buffers are expert-concatenated; advance by each split's row count.
  const float* amax_in_ptr = amax_input;
  float* amax_out_ptr = amax_output;

  MultiPaddingWithAmaxArgs kernel_args;
  kernel_args.num_tensors = 0;
  kernel_args.block_range[0] = 0;
  for (size_t tensor_id = 0; tensor_id < input_list.size(); ++tensor_id) {
    if (kernel_args.num_tensors == kMaxTensorsPerKernel) {
      TRANSFORMER_ENGINE_TYPE_SWITCH_ALL(
          type, Type, constexpr int nvec = desired_load_store_size / sizeof(Type);
          const int n_blocks = kernel_args.block_range[kernel_args.num_tensors];
          multi_padding_with_amax_kernel<nvec, Type>
          <<<n_blocks, threads_per_block, 0, stream>>>(kernel_args););  // NOLINT(*)
      NVTE_CHECK_CUDA(cudaGetLastError());
      kernel_args.num_tensors = 0;
    }
    ...
    kernel_args.input_list[pos] = const_cast<void*>(input_list[tensor_id]->data.dptr);
    kernel_args.output_list[pos] = output_list[tensor_id]->data.dptr;
    kernel_args.amax_input_list[pos] = const_cast<float*>(amax_in_ptr);
    kernel_args.amax_output_list[pos] = amax_out_ptr;
    kernel_args.num_rows_list[pos] = num_rows;
    kernel_args.padded_num_rows_list[pos] = padded_num_rows;
    kernel_args.row_length_list[pos] = row_length;
    kernel_args.block_range[pos + 1] = kernel_args.block_range[pos] + num_tiles;
    ...
    amax_in_ptr += num_rows;
    amax_out_ptr += padded_num_rows;
  }
...
void nvte_multi_padding_with_amax(size_t num_tensors, const NVTETensor* input_list,
                                  ...
                                  cudaStream_t stream) {
  NVTE_API_CALL(nvte_multi_padding_with_amax);
  using namespace transformer_engine;
  std::vector<Tensor*> input_list_, output_list_;
  std::vector<int> padded_num_rows_list_;
  for (size_t i = 0; i < num_tensors; ++i) {
  ...
  multi_padding_with_amax(input_list_, output_list_, amax_input, amax_output,
